...
if len(sys.argv) > 1:
    for i in range(1, len(sys.argv)):
        if not already_done:
            if sys.argv[i] == "-c":
                # asking to set the color
                if len(sys.argv) > i+1:
                    change_color(sys.argv[i+1])
                    already_done = True
                else:
                    print("ihm.py [-c color]")
            elif sys.argv[i] == "-t":
                # asking to set the text
                if len(sys.argv) > i+1:
                ...
                print("["+str(sys.argv[i])+"] is an unkown command.... Here are the known commands\nihm.py [-c color] [-t \"text\"]")
        else:
            already_done = False
